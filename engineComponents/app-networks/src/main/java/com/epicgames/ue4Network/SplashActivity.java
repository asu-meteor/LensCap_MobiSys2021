...
public class SplashActivity extends Activity
{
	...
	{
		/*
		final LensCapReceiver lensCap = new LensCapReceiver(this);
		Log.error(Boolean.toString(lensCap.getServiceConnected()));
		lensCap.registerReceiver(new LensCapReceiver.Receiver<byte[]>() {
			@Override
			public void onReceived(@NotNull String identifier, final byte[] data) {
				Boolean ah=lensCap.getServiceConnected();
				Log.error(ah.toString());
				SplashActivity.this.runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if (cha.compareTo("true") == 0) {
							Log.error("bello" + "bello");
						}
					}
				});
			}
